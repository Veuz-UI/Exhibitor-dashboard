!(function (a) {
            ...
            if (!n || f.options.allowDuplicates) {
              if (!(f.items().toString().length + b.length + 1 > f.options.maxInputLength)) {
                ...
                if ((f.$element.trigger(o), !o.cancel)) {
                  f.itemsArray.push(b);
                  var p = a(
                    '  <span style="position:relative;cursor:pointer;display:inline-flex;justify-content:center"  class="tag ' +
                      e(l) +
                      (null !== m ? '" title="' + m : '') +
                      '">' +
                      '<input type="color" name="color1" id="color1" oninput="changeColor(event)" onClick="activeTag(event)" style="width:100%;height:100%;position:absolute;left:0;cursor:pointer;z-index:100;opacity:0;" />' +
                      e(k) +
                      '<span data-role="remove" style="z-index:999"></span></span>'
                  );
                  p.data('item', b), f.findInputWrapper().before(p), p.after(' ');
                  setCustomTags(p); // custom function
                  var q =
  ...
  const setCustomTags = (tag) => {
    if (tag[0]) {
      selectedTagElement = tag[0];
    }
  };
  
  const activeTag = (e) => {
    const tagElement = e.target.closest('.tag');
    selectedTagElement = tagElement;
  };
  
  const changeColor = (event) => {
    if (selectedTagElement) {
      selectedTagElement.style.backgroundColor = event.target.value;
    }
  };